...
def prepare_dataset():
    ...
    # t_train = t_train.repeat(factor)
    # X_test = torch.cat([X_test + torch.randn_like(X_test) * 0.1 for _ in range(factor)])
    # t_test = t_test.repeat(factor)
...
def main(rank, world_size, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for features, targets in train_loader:
            ...
            features, targets = features.to(rank), targets.to(rank)  # New: use rank
            ...
            print(f"[GPU{rank}] Epoch: {epoch+1:03d}/{num_epochs:03d} | "
                  f"Batchsize {targets.shape[0]:03d} | "
    ...
    # NEW (not in the book):
        ...
        raise ZeroDivisionError(
            ...
            f"Or, to run it on {torch.cuda.device_count()} GPUs, uncomment the code on lines 100 to 103."
        )
    ####################################################
...
if __name__ == "__main__":
    ...
    # Run `CUDA_VISIBLE_DEVICES=0,1 python ddp_tutorial.py` if you have GPUs > 2
